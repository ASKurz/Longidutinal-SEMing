#title:  Newsom Longitudinal SEM Chapter 1, Example 1.3c
...
model1.3c <- #Referent identification;

	'w1unw =~ w1unw1d + w1unw2d + w1unw3d

w1unw ~~ w1unw

#Referent threshold identification;		
...
fitmodel1.3c <- sem(model1.3c, data = socex1.1, estimator= "WLSMV", parameterization = theta, ordered=c("w1unw1d", "w1unw2d", "w1unw3d"))

summary (fitmodel1.3c, fit.measures= TRUE, rsquare= TRUE, standardized= TRUE)

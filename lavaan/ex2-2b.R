#title:  Newsom Longitudinal SEM Chapter 2, Example 2.2b
...
#+++++++++++++++++++++++++++++++
# Chapter 2, Example 2.2b 
#+++++++++++++++++++++++++++++++

model2.2b <- #alternative referent;

	'w1comp =~ NA*w1vst1 + w1vst2 + 1*w1vst3
	     
	 w2comp =~ NA*w2vst1 + w2vst2 + 1*w2vst3

	#variances and covarainces
		w2comp ~~ w1comp

		w2comp ~~ a*w2comp #factor variance equality constraint;
		w1comp ~~ a*w1comp
		w1vst1 ~~ w2vst1 
	     	w1vst2 ~~ w2vst2 
	     	w1vst3 ~~ w2vst3'

fitmodel2.2b <- sem(model2.2b, socex1.1)

summary (fitmodel2.2b, fit.measures= TRUE, rsquare= TRUE, standardized= TRUE)
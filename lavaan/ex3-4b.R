#title:  Newsom Longitudinal SEM Chapter 3, Example 3.4b
...
#+++++++++++++++++++++++++++++++
# Chapter 3, Example 3.4b  SRMR is different (almost half)--doesn't matter?
#+++++++++++++++++++++++++++++++
...
model3.4b <- '	int =~ 1*w1posaff + 1*w2posaff + 1*w3posaff
		d1 =~ 0*w1posaff + 1*w2posaff + 0*w3posaff
		d2 =~ 0*w1posaff + 0*w2posaff + 1*w3posaff
		...
		int ~ 1  
		d1 ~ 0  #constrain d1 and d2 means to 0 for omnibus test;
		d2 ~ 0
		int ~~ int
		d1 ~~ 0*d1
		d2 ~~ 0*d2
...
fitmodel3.4b <- sem(model3.4b, data=socex1.1)
summary(fitmodel3.4b, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
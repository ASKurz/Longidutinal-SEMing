#title:  Newsom Longitudinal SEM Chapter 3, Example 3.7c
...
model3.7c <- '  #group 1  [1] married, group 2 [0] not married in output

		w1posaff =~ NA*w1happy + a*w1happy + b*w1enjoy + c*w1satis + d*w1joyful + e*w1please
...
fitmodel3.7c <- sem(model3.7c, data=socex1.1, estimator="ML", information="observed", 
		group="w1marr2", group.equal=c("intercepts", "loadings"))

summary(fitmodel3.7c, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)

#title:  Newsom Longitudinal SEM Chapter 5, Example 5.2b
...
model5.2b <- '  #common labels for loadings imposes longitudinal equality constraints;
		#referent identification--first loading is referent by default;
	      ...
	      w2unw =~ w2unw1 + (lambda7)*w2unw2 + (lambda8)*w2unw3
...
fitmodel5.2b <- sem(model5.2b, data=socex1.1, information="expected")
summary(fitmodel5.2b, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)

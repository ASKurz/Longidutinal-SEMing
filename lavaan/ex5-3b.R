#title:  Newsom Longitudinal SEM Chapter 5, Example 5.3b
...
#+++++++++++++++++++++++++++++++
# Chapter 5, Example 5.3b 
#+++++++++++++++++++++++++++++++

model5.3b <-   ' #common labels for loadings imposes longitudinal equality constraints;
		...
		w1posaff =~ NA*w1happy + (lambda8)*w1happy + (lambda9)*w1enjoy + (lambda10)*w1satis + (lambda11)*w1joyful + (lambda12)*w1please
...
#Model constraints
	...
	lambda8 == 5 - lambda9 - lambda10 - lambda11 - lambda12'

fitmodel5.3b <- sem(model5.3b, data=socex1.1, parameterization="theta", estimator="wlsmv", 
		ordered=c("w1dboth", "w1dsad", "w1dblues", "w1ddep", "w2dboth", "w2dsad", "w2dblues", "w2ddep"))
summary(fitmodel5.3b, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
